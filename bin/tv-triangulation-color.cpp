...
int main(int argc, char **argv)
{
  ...
  general_opt.add_options()("help,h", "display this message")("input,i", po::value<std::string>(), "Specifies the input shape as a 2D image PPM filename.")("output,o", po::value<std::string>()->default_value("after-tv-opt"), "Specifies the basename of output displayed images. See option -D, file name is completed by -g, -lg, -pu, -2nd, -2nd-with-slices depending on display choice.")("limit,L", po::value<int>()->default_value(200), "Gives the maximum number of passes (default is 200).")("bitmap,b", po::value<double>()->default_value(2.0), "Rasterization magnification factor [arg] for PNG export.")("strategy,s", po::value<int>()->default_value(4), "Strategy for quadrilatera with equal energy: 0: do nothing, 1: subdivide, 2: flip all, 3: flip all when #flipped normal = 0, 4: flip approximately half when #flipped normal = 0, 5: flip approximately half when #flipped normal = 0, subdivide if everything fails.")("tv-power,p", po::value<double>()->default_value(0.5), "The power coefficient used to compute the gradient ie |Grad I|^{2p}. ")("lambda,l", po::value<double>()->default_value(0.0), "The data fidelity term in TV denoising (if lambda <= 0, then the data fidelity is exact")("dt", po::value<double>()->default_value(0.248), "The time step in TV denoising (should be lower than 0.25)")("tolerance,t", po::value<double>()->default_value(0.01), "The tolerance to stop the TV denoising.")("quantify,q", po::value<int>()->default_value(256), "The quantification for colors (number of levels, q=2 means binary.")("tv-max-iter,N", po::value<int>()->default_value(20), "The maximum number of iteration in TV's algorithm.")("nb-alt-iter,A", po::value<int>()->default_value(1), "The number of iteration for alternating TV and TV-flip.")("display-tv,d", po::value<int>()->default_value(0), "Tells the display mode after standard TV regularization per bit: 0x1 : output Flat colored triangles, 0x2 : output Gouraud colored triangles, 0x4: output Linear Gradient triangles.")("display-flip,D", po::value<int>()->default_value(4), "Tells the display mode aftergeometric TV flips per bit: 0x1 : output Flat colored triangles, 0x2 : output Gouraud colored triangles, 0x4: output Linear Gradient triangles (quite nice and flat), 0x8: output partition of unity Bezier triangles (slow and not good), 0x10: output 2nd order reconstruction with Bezier curve (nicest but slower than 0x4), 0x20: same as 0x8 but displays also the similarity/discontinuity graph (for debug/illustrations), 0x40: use Poisson reconstruction to reconstruct linear gradient with discontinuities.")("discontinuities,S", po::value<double>()->default_value(0.1), "Tells to display a % of the TV discontinuities (the triangles with greatest energy).")("stiffness", po::value<double>()->default_value(0.9), "Tells how to stiff the gradient around discontinuities (amplitude value is changed at stiffness * middle).")("amplitude", po::value<double>()->default_value(1.0), "Tells the amplitude of the stiffness for the gradient around discontinuities.")("similarity", po::value<double>()->default_value(0.0), "Tells when two colors are considered identical for connectedness.")("connectivity", po::value<std::string>()->default_value("Order"), "Indicates the strategy for determining the connectivity of ambiguous pixels: Size | Order. Size is best for 1-bit images, Order is best for color images.")("debug", "specifies the DEBUG mode: output some intermediate results in cc.ppm and order.ppm.")("displayMesh", "display mesh of the vectorial display.")("exportVectMesh,e", po::value<std::vector<std::string>>()->multitoken(), "Export the triangle mesh.")("exportVectMeshDual,E", po::value<std::vector<std::string>>()->multitoken(), "Export the triangle mesh.")("exportVectContoursDual,C", po::value<std::vector<std::string>>()->multitoken(), "Export the image regions filled. Use 3rd bezier curves for the edges")("vectScale", po::value<double>(), "Change the default vectorial scale to ajust default display size. By default the scale is given from the Rasterization magnification factor (--bitmap,b option) (using 10 will display easely small images while 1.0 is more adapted to bigger images).")("numColorExportVectDual", po::value<unsigned int>()->default_value(0), "num of the color of the map.")("regularizeContour,R", po::value<int>()->default_value(20), "regularizes the dual contours for <nb> iterations.")("zip,z", po::value<double>()->default_value(1.0), "Compresses the triangulation to keep only the given proportion of vertices.")("zip-method,Z", po::value<std::string>()->default_value("Laplacian"), "zip method in Laplacian | Merge.")("version", "Display the version number.")
